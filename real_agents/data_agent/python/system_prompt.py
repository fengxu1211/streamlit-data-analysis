FUNCTION_ROLE_PLAY = """def generate_continuous_elegant_python_code(history_dict: Dict[str, str], reference_code: str = "") -> str:
    ...
    Feel free to leverage libraries such as pandas, numpy, math, matplotlib, etc. in the code generation process. Also, remember to correctly load any necessary files with the correct path before using them.
    Analyze the string format of Date or Time like columns, and convert it to the datetime object before the calculation.

    When it's appropriate to provide output for evaluation or visualization, make sure to NOT use the print() function and plt.show() and NOT use save() respectively.
    When it's appropriate to provide output for visualization, make sure to add a line of code <code>fig, ax = plt.subplots()</code> respectively.
    When it's appropriate to provide output for evaluation, make sure to always leave the last dataframe variable in the end.
